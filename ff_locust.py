...
def hook_request_success(request_type, name, response_time, response_length, **kw):
    ff_log(ff_metric("request_success", 
...
def hook_request_fail(request_type, name, response_time, response_length, exception, **kw):
    ff_log(ff_metric("request_failure",
...
def hook_spawning_complete(user_count, **kw):
    ff_log(ff_metric("spawning_complete",
...
def hook_test_stop(**kw):
    ff_log(ff_metric("test_stop", {"count": 1}))
...
def hook_test_start(**kw):
    ff_log(ff_metric("test_start", {"count": 1}))
...
def hook_user_error(user_instance, exception, tb, **kw):
    # print('FF EXCEPTION', exception)
    ff_log(ff_metric("user_error",
...
def hook_worker_report(client_id, data, **kw):
    ff_log(ff_metric("worker_report",
...
def hook_report_to_master(client_id, data, **kw):
    ff_log(ff_metric("report_to_master",
        data, {"client_id": client_id}))
...
def hook_init(environment, **kw):
    ff_log(ff_metric("init", {"count": 1}, {"url": environment.host}))

# Create ff_metric
# Outputs JSON in ff_metric format
def ff_metric(measurement, fields = None, tags = None):
